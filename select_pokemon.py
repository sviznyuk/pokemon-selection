...
df1=pokemon_type_df.merge(damage_relations_df,how='left',left_on='pokemon_type',right_on='from_type')
df2=df1.merge(damage_relations_df,how='left',left_on='pokemon_type',right_on='to_type')
df3=df2.drop(columns=['from_type_x','to_type_y'])
df4=df3.rename(columns={'damage_x':'damage_to','to_type_x':'to_type','damage_y':'damage_from','from_type_y':'from_type'})
df4.to_csv('pokemon_damage_levels.csv',index=False,header=True)
df5=df4.groupby('pokemon_name')['damage_to','damage_from'].sum().reset_index()
df5['damage_diff']=df5['damage_to']-df5['damage_from']
df6=df5.sort_values(by=['damage_diff','damage_to'],ascending=False)
df6.to_csv('pokemon_ranking.csv',index=False,header=True)
